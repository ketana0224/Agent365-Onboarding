...
> 親: [Handson README](../README.md) ／ 前: [lab4｜Agent ID 出口化と統合ガバナンス](../lab4/Lab1-5_extLab2をA365フル機能化.md)

## このステップの狙い

lab3 / lab4 では、エージェントの出口（LLM / MCP）を **Agent ID（fmi_path 2 ステップ交換の Step 2a＝自律型）** に差し替えた。自律型は「エージェント自身の権限」で動くため、**誰が呼んでも同じ権限**になる。
...
| 効く統制 | 自律型（lab3/lab4・Step 2a） | OBO（lab5・Step 2b） |
|---|---|---|
...
| 項目 | Step 2a（自律型 / lab3・lab4） | Step 2b（OBO / lab5） |
|---|---|---|
...
自律型ホスト（lab3/lab4）との差分は **「OBO 経路の 3 点」** だけ。LLM / MCP の自律型出口（Step 2a）はそのまま残っている。
...
| lab3 / lab4 の理解 | 出口の Agent ID 差し替え（Step 2a）を体験済みであること。OBO は同じ基盤の Step 2b |
...
> #1（Agent ID 側）と #2〜#5（ユーザー側）は **独立に効く**。これが「二重統制」の実体。自律型（lab3/lab4）は #1 しか効かないが、OBO は #1〜#6 すべてが遮断ポイントになる。
...
> 観測は2段階: **直後** = トークン発行成功＋Graph `401 Authorization_IdentityDisabled` → **数分後** = Step 1 の発行自体が `AADSTS7000112` で失敗。詳細は lab4 のキルスイッチ検証と同じ。
...
| 観点 | 自律型（Step 2a / lab3・lab4） | OBO（Step 2b / lab5） |
|---|---|---|
