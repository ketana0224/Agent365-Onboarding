...
<#
.SYNOPSIS
  agent-custom-MAF-ACA-A365-obo 用の .env を .env.example から自動生成する。

.DESCRIPTION
  lab3 の prepare-env.ps1（egress 版）と同じ仕組みで .env を生成し、OBO 版の値を差し込む:
    AZURE_TENANT_ID         … lab2 の a365.config.json（無ければ az account show）
    AZURE_SUBSCRIPTION_ID   … az account show
    BLUEPRINT_APP_ID        … lab2 generated config の agentBlueprintId
    AGENT_IDENTITY_APP_ID   … lab2 generated config の agenticAppId
    BLUEPRINT_CLIENT_SECRET … agentBlueprintClientSecret を DPAPI(CurrentUser) 復号
    BLUEPRINT_API_AUDIENCE  … api://<BLUEPRINT_APP_ID>（/obo-chat が受け入れる aud）
    USE_AGENT_ID_EGRESS     … true（自律型出口 + OBO で Agent ID 構成を共有）
    ACA_RESOURCE_GROUP      … rg-<Me>（lab2 と同じものを再利用）
    ACA_APP_NAME            … custom-maf-a365-obo-<Me>（ACA 名 32 文字制限のため agent を省く）
...
param(
    ...
    [string]$Me,
    # lab2 で a365 setup all を実行した config ディレクトリ
...
Write-Host "== agent-custom-MAF-ACA-A365-obo 用 .env を生成 ($Me) ==" -ForegroundColor Cyan
...
if ((Test-Path $cfgPath) -and (Test-Path $genPath)) {
    ...
    Write-Host "      lab2 config から Agent ID 値を取得: $SourceDir" -ForegroundColor Green
}
else {
    Write-Host "[warn] lab2 の config が見つかりません（$SourceDir）。Agent ID 値は空のままにします。" -ForegroundColor Yellow
    Write-Host "       lab2 で a365 setup all を実行済みか、-SourceDir を確認してください。" -ForegroundColor Yellow
}
...
# obo 版は app 名で区別する）。BLUEPRINT_API_AUDIENCE は Blueprint の identifierUri に合わせる。
$overrides = @{
    'USE_AGENT_ID_EGRESS' = 'true'
    'GRAPH_SCOPE'         = 'https://graph.microsoft.com/.default'
    'ACA_RESOURCE_GROUP'  = "rg-$Me"
    'ACA_APP_NAME'        = "custom-maf-a365-obo-$Me"
    'ACA_ENV_NAME'        = "aca-contoso-agent-$Me"
}
if ($TenantId)       { $overrides['AZURE_TENANT_ID']        = $TenantId }
if ($SubscriptionId) { $overrides['AZURE_SUBSCRIPTION_ID']  = $SubscriptionId }
if ($BlueprintAppId) {
    $overrides['BLUEPRINT_APP_ID']      = $BlueprintAppId
    $overrides['BLUEPRINT_API_AUDIENCE'] = "api://$BlueprintAppId"
}
...
Write-Host "  BLUEPRINT_API_AUDIENCE : $(if ($BlueprintAppId) { "api://$BlueprintAppId" } else { '<空・要手入力>' })"
...
Write-Host "  USE_AGENT_ID_EGRESS    : true（自律型出口 + OBO で Agent ID 構成を共有）" -ForegroundColor DarkGray
Write-Host "  GRAPH_SCOPE            : https://graph.microsoft.com/.default" -ForegroundColor DarkGray
Write-Host "  ACA_RESOURCE_GROUP     : rg-$Me"
Write-Host "  ACA_APP_NAME           : custom-maf-a365-obo-$Me"
Write-Host "  ACA_ENV_NAME           : aca-contoso-agent-$Me"
Write-Host ""
Write-Host "OBO を使う前に scripts\01〜03 を実行して Blueprint の OAuth API 化 / OBO クライアント /" -ForegroundColor DarkGray
Write-Host "Agent Identity の Graph 委任付与を済ませてください。" -ForegroundColor DarkGray
