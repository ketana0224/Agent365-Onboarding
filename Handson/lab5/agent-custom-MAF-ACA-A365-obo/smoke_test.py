"""デプロイした MAF エージェント（ACA / Agent ID 出口化 + OBO 版）の疎通スモークテスト。
/healthz と /chat（自律型）にサンプル質問を投げて応答を表示する。
標準ライブラリのみで動作（追加依存なし）。

使い方:
    python smoke_test.py [BASE_URL]
    例: python smoke_test.py https://custom-maf-a365-obo.xxxx.eastus2.azurecontainerapps.io
    既定: http://localhost:8000

OBO（/obo-chat）はユーザー サインインが必要なため、このスモークテストではなく
scripts\test-obo-end-to-end.ps1 または chat-ui-obo（Streamlit）で検証する。
...
# 自律型（/chat）の動作確認: MCP ポリシーに基づく決定的回答
QUESTIONS = [
...
def main(base: str) -> int:
    ...
    # 出口トークンの種別を確認（Agent ID 出口化 / OBO の検証）
    try:
        dbg = json.loads(_get(f"{base}/debug/auth"))
        print(f"\n== /debug/auth use_agent_id_egress={dbg.get('use_agent_id_egress')}")
        for ev in dbg.get("events", [])[-6:]:
            print(f"   - {ev.get('phase')} aud={ev.get('token', {}).get('aud')} appid={ev.get('token', {}).get('appid')}")
    except Exception:  # noqa: BLE001
        pass

    print("\nOBO（/obo-chat）の検証はユーザー サインインが必要です:")
    print("  pwsh .\\scripts\\test-obo-end-to-end.ps1 -BaseUrl", base)
    print("  またはブラウザ UI: ..\\chat-ui-obo\\app.py（Streamlit）")
    return 0
