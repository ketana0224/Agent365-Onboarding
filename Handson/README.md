...
> **コア体験は Lab0→Lab4**（Agent ID + 出口統制 + キルスイッチ）。Lab6（Observability）は任意トラック。
...
| **やること** | ・`USE_AGENT_ID_EGRESS=true` で出口を Agent ID に切替（Blueprint シークレットは Key Vault 参照で ACA に供給）<br>・**(a) Agent ID 停止**（無効化 / CA / 削除）で LLM / MCP が止まることを検証<br>・(b) APIM の audience 検証・(c) A365 ネイティブ観測性を補助確認 |
...
| **メモ** | 深い per-span トレースは**コードアクセス依存（②）**。①Identity / ガバナンスは Lab2 以降コード非依存で効いている。 |
...
Lab0 → Lab2（Agent ID + CA ブロック）→ Lab3（UAMI/APIM 出口集約）→ Lab4（キルスイッチ）。Lab1 は概念説明、Lab5（OBO）/ Lab6（Observability）はデモ提示で代替。
...
- **素体は作り直さない**。Lab2 以降は同じ `custom-maf-agent-a365` を強化していく（Lab3 で UAMI/APIM 配線、Lab4 で Agent ID 出口、Lab6 で Observability Distro 初期化）。
- **Agent ID と実行体は別物**。CA で止まるのは「ID としてのアクセス」、プロセス停止は ACA 操作（自作の場合）。
...
| 内容 | 参照 |
|---|---|
| Lab0 オリエン & 環境確認 | [lab0/Lab0_README.md](lab0/Lab0_README.md) |
