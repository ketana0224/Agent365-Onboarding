...
flowchart LR
    subgraph S1["Lab"]
        ...
        Lab7["Lab7: M365 到達性<br/>(Teams で往復)"]
        Lab8["Lab8: AI teammate<br/>(人間相当の統制)"]
        Lab9["Lab9: Foundry Hosted 化(任意)<br/>(マネージド ホスティング)"]
    ...
    subgraph S2["Copilot Studio と共通か"]
        ...
        C8["✅ 共通: Observability は標準<br/>(深い計装は自作コードのみ)"]
...
| Lab6 | **行動を可観測化**（per-span トレースを Defender / Purview / 管理センターに）。統制レベルは上げず**テレメトリ**を足す | Lab2 と同一の Agent ID（スパンの `{agentId}` ＝ インスタンス appId） |
| Lab7 | Lab2 の統制 + **Teams 到達性** | Lab2 と同一の Agent ID + メッセージング エンドポイント |
| Lab8 | **人間社員相当**（メール / Teams 在席 / 上長関係 / DLP） | 専用ユーザー（agentic user・`aiTeammate=true`） |
| Lab9 | ホスティング・スケール・Playground を Foundry に寄せる | Hosted Agent のマネージド ID |
...
| **Lab6** | [A365 Observability で行動を可観測化する](#lab6a365-observability-で行動を可観測化する) | 起動時の OpenTelemetry Distro 初期化 1 点で、エージェントの per-span な行動を Defender / Purview / 管理センターに可観測化 | [lab6-1](lab6/lab6-1_A365Observability.md) | 40 分 |
| **Lab7** | [Teams から呼べるようにする](#lab7teams-から呼べるようにする) | メッセージング エンドポイント登録で Teams 往復 | [Lab1-3](lab7/Lab1-3_m365.md) / [extLab2-3](lab7/extLab2-3_Teams接続_M365AgentsSDK.md) | 45 分 |
| **Lab8** | [AI teammate にする](#lab8ai-teammate-にする) | 専用ユーザー（agentic user）で人間相当の統制 | [Lab1-4](lab8/Lab1-4_AIteammate.md) | 40 分 |
| **Lab9**（任意） | [Foundry Hosted Agent に載せ替える](#lab9任意foundry-hosted-agent-に載せ替える) | マネージド ホスティングへ lift、Block 検証 | [lab-foundry-hosted-agent/README](lab9/lab-foundry-hosted-agent/README.md) | 40 分 |

> **コア体験は Lab0→Lab4**（Agent ID + 出口統制 + キルスイッチ）。Lab6（Observability）/ Lab7（Teams 到達性）/ Lab8（AI teammate）は任意トラック、Lab9 は「自前 ACA 運用」と「Foundry マネージド運用」の比較。
...
### Lab6｜A365 Observability で行動を可観測化する

| 項目 | 内容 |
|---|---|
| **ねらい** | 統制レベルは Lab2 のまま上げず、**エージェントが実際に何をしたか**を **Defender / Purview / Microsoft 365 管理センター** の 3 面で可観測化する（テレメトリトラック）。**格納先は Azure Monitor ではなく Defender 基盤**で、計装は Distro 1 点・宛先は 2 プレーン（運用＝Azure Monitor / 統制＝Defender）として理解する。 |
| **やること** | ・既存エージェント（OBO / egress 版）の起動時に **Microsoft OpenTelemetry Distro** を A365 有効で初期化（1 点）<br>・`ENABLE_A365_OBSERVABILITY_EXPORTER=true` / `AGENT365OBSERVABILITY__AGENTID`（インスタンス appId）を ACA に転記<br>・1〜2 往復会話して `invoke_agent` / `chat` / `execute_tool` スパンを発生させる<br>・Defender `CloudAppEvents`（KQL）・管理センター インベントリ・Azure Monitor（`trace_id` 相関）の 3 面で検証 |
| **成果物 / 検証** | コードを足したエージェントの per-span な行動が Defender / 管理センターに出る。Azure Monitor と `trace_id` で相関し 2 プレーンが 1 本のランとして繋がる |
| **対応 lab** | [lab6-1](lab6/lab6-1_A365Observability.md)（設計根拠: [Observability_DirectOTel_と格納先.md](Observability_DirectOTel_と格納先.md)） |
| **メモ** | 深い per-span トレースは**コードアクセス依存（②）**。①Identity / ガバナンスは Lab2 以降コード非依存で効いている。M365 インタラクションが出る **Lab7（Teams）後**は Purview / Defender への自動収録がさらに分かりやすい。 |

### Lab7｜Teams から呼べるようにする
...
| **対応 lab** | [Lab1-3](lab7/Lab1-3_m365.md) / [extLab2-3](lab7/extLab2-3_Teams接続_M365AgentsSDK.md) |

### Lab8｜AI teammate にする
...
| **対応 lab** | [Lab1-4](lab8/Lab1-4_AIteammate.md) |
| **メモ** | (3) `--aiteammate` は **Frontier プログラム + 対応ライセンス**前提。未契約なら Lab7 までで止め、手順は承認待ちとして提示。 |

### Lab9（任意）｜Foundry Hosted Agent に載せ替える
...
| **対応 lab** | [lab-foundry-hosted-agent/README](lab9/lab-foundry-hosted-agent/README.md) |
...
| （昼休み） | |
| 4:30–5:10 | Lab6 A365 Observability（可観測化） |
| 5:10–5:55 | Lab7 Teams 到達性 |
| 5:55–6:35 | Lab8 AI teammate |
| （任意） | Lab9 Foundry Hosted 化 |

### 短縮版（半日・約 3h｜統制の本質に集中）

Lab0 → Lab2（Agent ID + CA ブロック）→ Lab3（UAMI/APIM 出口集約）→ Lab4（キルスイッチ）。Lab1 は概念説明、Lab5（OBO）/ Lab6（Observability）/ Lab7（Teams）/ Lab8（AI teammate）/ Lab9 はデモ提示で代替。
...
- **素体は作り直さない**。Lab2 以降は同じ `custom-maf-agent-a365` を強化していく（Lab3 で UAMI/APIM 配線、Lab4 で Agent ID 出口、Lab6 で Observability Distro 初期化、Lab7 で `/api/messages` 実装）。
- **Agent ID と実行体は別物**。CA で止まるのは「ID としてのアクセス」、プロセス停止は ACA 操作（自作の場合）。Hosted（Lab9）は ID と実行を一体で統制できる。
- **Frontier 依存の段（Lab1 の Registry sync・Lab8 の AI teammate）** は未契約環境ではデモ提示に切り替える。
...
| Foundry Hosted Agent 化 | [lab9/lab-foundry-hosted-agent/README.md](lab9/lab-foundry-hosted-agent/README.md) |
| カスタム→フル機能化の差分 | [_report/extLab2_agent差分_カスタムからフル機能化.md](../extLab2_agent差分_カスタムからフル機能化.md) |
| Observability の格納先（Lab6 設計根拠） | [Observability_DirectOTel_と格納先.md](Observability_DirectOTel_と格納先.md) |
