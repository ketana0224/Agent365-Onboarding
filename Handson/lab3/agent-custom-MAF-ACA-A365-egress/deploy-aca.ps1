...
<#
.SYNOPSIS
    MAF ホスト型エージェント（Agent ID 出口化版）を Azure Container Apps にデプロイします。

.DESCRIPTION
    B（agent-custom-MAF-ACA-A365）の deploy-aca.ps1 と同一仕様（Contoso サポート・
    同一指示・同一 MCP ツール・同一モデル・APIM 経由）に加え、Agent ID 出口化
    （fmi_path 2 ステップ交換）の設定を投入します。

    1. .env を読み込み（PROJECT_ENDPOINT / モデル / APIM / MCP / App Insights /
       Agent ID 出口化（AZURE_TENANT_ID / BLUEPRINT_APP_ID / AGENT_IDENTITY_APP_ID /
       BLUEPRINT_CLIENT_SECRET / USE_AGENT_ID_EGRESS）など）
...
.NOTES
    Agent ID 出口化を有効化するには、a365 setup all で Blueprint / Agent Identity を発行済みで、
    .env に BLUEPRINT_APP_ID / AGENT_IDENTITY_APP_ID / BLUEPRINT_CLIENT_SECRET を設定してください
    （B と同じ Agent ID を再利用できます）。
    出口の切替（USE_AGENT_ID_EGRESS）は起動時に評価されるため、変更後はリビジョン再起動が必要です。
#>
...
Write-Host '== MAF ホスト型エージェント（Agent ID 出口化版）デプロイ (Azure Container Apps) ==' -ForegroundColor Cyan
...
if ($egressOn) {
    foreach ($pair in @(
        ...
        if (-not $pair.Value) {
            throw "USE_AGENT_ID_EGRESS=true ですが .env に $($pair.Name) がありません。"
        }
    }
}
...
Write-Host "出口モード  : USE_AGENT_ID_EGRESS=$useAgentIdEgress" -ForegroundColor Green
Write-Host ''
