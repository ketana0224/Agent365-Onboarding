...
=====================================================================
このフォルダ（agent-custom-MAF-ACA-A365-egress）直下の .env を読み込む。
ローカル実行では scripts/setup-env が生成した .env を使い、ACA 上では
Container App の環境変数（deploy-aca.ps1 が設定）から同じキーを読み取る。

B（agent-custom-MAF-ACA-A365）と同一の接続情報・MCP 設定・モデルを使用する。
差分は **Agent ID 出口化**（fmi_path 2 ステップ交換）の設定のみ:
    USE_AGENT_ID_EGRESS / AZURE_TENANT_ID / BLUEPRINT_APP_ID /
    AGENT_IDENTITY_APP_ID / BLUEPRINT_CLIENT_SECRET
"""
...
# agent-custom-MAF-ACA-A365-egress/.env を読み込む（無ければ環境変数のみ使用）
