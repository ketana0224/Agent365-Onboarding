"""
MAF エージェント定義（ホスト型 / カスタム / APIM AI Gateway 経由 + Agent ID 出口化版）
=========================================================
B（agent-custom-MAF-ACA-A365）と同一のエージェント本体（同一指示・同一 MCP ツール・
同一モデル・APIM 経由）に、**Agent ID 出口化**（fmi_path 2 ステップ交換）を追加した版。
  ...
  - MCP: `streamablehttp_client(CONTOSO_MCP_URL, headers=Bearer)` で
         APIM の `/contoso-policy/mcp` を呼ぶ。

認証（出口）:
  - USE_AGENT_ID_EGRESS=false（既定）: マネージド ID（DefaultAzureCredential）が
    `https://cognitiveservices.azure.com/.default` の token を取得して APIM を通る。
  - USE_AGENT_ID_EGRESS=true: マネージド ID の代わりに Agent ID（fmi_path 2 ステップ
    交換）のトークンを出口に使う。LLM・MCP の双方が Agent Identity SP として下流を
    呼ぶため、Entra Agent ID の統制（CA / Disable / Block）が実トラフィックに効く。
    トグルは起動時に評価される（切替には ACA リビジョン再起動が必要）。

注意:
  - 本版は `/chat`（自律型）のホストで、ユーザー委任（OBO）や Teams 連携は含まない。
    そのため出口トークンは常に **自律型（Step 2a）** で取得する。OBO（Step 2b）は
    agent_id_token.py に実装は残すが、本ホストからは呼び出さない。
"""
...
AGENT_NAME = "custom-maf-agent-a365-egress"
...
class AgentIdCredential:
    ...
    交換）のリソーストークンを返す。これで LLM 出口が Agent Identity SP になる。
    """
...
async def _mcp_headers() -> dict[str, str]:
    """MCP 呼出ヘッダー。APIM 経由は Bearer（出口トークン）、切り戻し時のみ legacy x-contoso-key。"""
    if _mcp_uses_bearer():
...
async def build_agent(stack: AsyncExitStack):
    ...
    # 出口を Agent ID にする場合でも、MCP の legacy フォールバックや切り戻しのために
    # DefaultAzureCredential は常に確保しておく。
    ...
    #
    # USE_AGENT_ID_EGRESS=true のときは、UAMI の代わりに Agent ID（fmi_path 2 ステップ
    # 交換）のトークンを返す AgentIdCredential を渡す。これで LLM 出口が Agent Identity SP
    # になり、Entra Agent ID の統制が実トラフィックに効く。
    if config.use_agent_id_egress():
