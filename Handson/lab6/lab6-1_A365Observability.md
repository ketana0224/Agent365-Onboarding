...
```powershell
# ① スパンを 1 本作る（CONTAINER_FQDN は deploy-aca.ps1 出力の URL）
$body = @{ message = "observability check" } | ConvertTo-Json
Invoke-RestMethod -Method Post -Uri "https://<CONTAINER_FQDN>/chat" -ContentType 'application/json' -Body $body
```

```kusto
// ② A365 OTLP export の HTTP ステータス（200=着弾、4xx/skip=不着）
ContainerAppConsoleLogs_CL
| where ContainerAppName_s == "custom-maf-a365-obo-user99"   // 自分の ACA 名
...
> export 200 は通過点。`az containerapp logs show -g rg-user99 -n custom-maf-a365-obo-user99 --tail 80 --follow` でストリーム可。次に §4.2 で着弾を確定する。

### 4.2 本合否｜Defender CloudAppEvents 着弾（〜5分・ライセンス割当だけで到達）

> 前提＝**lab7-2 §2.1** の Defender XDR プレビュー＋AI エージェント セキュリティ ON（`CloudAppEvents` 自体の有効化）。**MDA 完全オンボードは不要**。受理されたスパンはこの1テーブルが格納先。export 200 の数分後に行が出れば lab6 合格。`KS204` が続くなら受理されていない＝§4.1 へ戻り 403/operation名/skip を直す。
...
```kusto
AgentsInfo
| where EntraAgentID == "<Agent Identity appId>"
| project Timestamp, Name, EntraAgentID, PublishedStatus, LifecycleStatus
```
...
$rg="rg-user99"; $loc="eastus2"; $ws="workspace-rguser99TdFM"
