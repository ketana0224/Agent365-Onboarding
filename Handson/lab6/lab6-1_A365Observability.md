...
その後、[chat-ui-obo](../lab5/chat-ui-obo/README.md)（lab5 と同じ OBO 用 UI）で **1〜2 往復**会話し、`invoke_agent`（ルート）/ `chat` / `execute_tool` に加えて **OBO ツール（`get_my_profile`）のスパン**まで発生させる。この計装済みスパン ツリーが §4 の検証対象になる。lab6 は OBO 版なので会話の入口は `/obo-chat`（`Authorization: Bearer <user_token>` 必須）。`local-chat-app` はユーザートークンを載せないため OBO の往復にはならない。

> 動作確認だけならトークン不要の `POST /chat`（自律型）でも基本スパン（`invoke_agent`/`chat`/`execute_tool`）は出る。ただし **OBO ツールのスパンは出ない**ため、検証は上記 chat-ui-obo（`/obo-chat`）で行う。


```powershell
# chat-ui-obo（Streamlit）を起動するだけの一括手順（venv はリポジトリ ルートを共用）
...
**手順**: ① §3 の chat-ui-obo でスパンを生成済み（直近のスパンが無ければ chat-ui-obo でもう一度往復する）→ ② `ContainerAppConsoleLogs_CL` で export 行を見る。

> **どこで見るか**: **ACA のコンソールログ**（`ContainerAppConsoleLogs_CL`）。Azure Portal → 対象の **Container App**（`custom-maf-a365-obo-userNN`）→ **監視 > ログ（Logs）** を開いて下の KQL を実行する（＝ACA 環境にひもづく Log Analytics ワークスペースを検索する）。GUI を開かず CLI で流し見るなら `az containerapp logs show -g rg-userNN -n custom-maf-a365-obo-userNN --tail 80 --follow`。Defender（`CloudAppEvents`）を見るのは §4.2。
...
> **どこで見るか**: Azure Portal の**検索ボックスで「Application Insights」を検索**し、**共有の Application Insights** リソース（`.env` の `APPLICATIONINSIGHTS_CONNECTION_STRING` が指す先。名前が分からなければ講師共有の RG 内 `appi-*`）を開く → **監視 > ログ（Logs）** で下の KQL を実行する。ガント チャートで見るなら同リソースの **調査 > トランザクションの検索**（後述の 💡）。
